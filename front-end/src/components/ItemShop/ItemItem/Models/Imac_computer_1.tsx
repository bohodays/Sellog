...
export function Imac_computer_1(props: JSX.IntrinsicElements["group"] | any) {
  const { nodes, materials } = useGLTF(
    "/models/items/imac_computer_1.glb"
  ) as GLTFResult;

  // 좌표 (서버에 저장된 좌표로 수정하기)
  const [position, setPosition] = useState({ x: 0, y: -4.2, z: 0 });
      ...
      if (isDragging) {
        ...
        if (found.length > 0) {
          for (let i = 0; i < found.length; i++) {
            ...
            setPosition({ x: newPosition.x, y: -4.2, z: newPosition.z });
            break;
          }
        }
      }
  ...
  return (
    <group
      ...
      userData={{ draggable: true, name: "Imac_computer_1" }}
      onClick={() => {
        if (props.activePage === "myitems") {
          if (!isDragging) setIsDragging(true);
          setTarget("Imac_computer_1");
        }
      }}
